...
def calculate():
    ...
    # Запуск оптимізації
    initial_guess = np.ones(20) * optimizer.Cin_max
    res = minimize(optimizer.objective_minimum_effort, initial_guess, args=(t_fix, 20), method='SLSQP')
    
    return jsonify({
        "model": "Оптимальне керування процесом очищення водойми",
        "author": "Брагар Софія, АІ-233",
        "input_t_fix": t_fix,
        "optimal_cost": round(res.fun, 4),
